...
def convert_cdc(w):
    ...
    print("conv_weight shape:", conv_shape)
...
saved_model_path = '/data/zhs/LightenDiffusion/savemodel/model_checkpoint_1204-UIEBD.pth.tar'
ckp = torch.load(saved_model_path)
ckp_keys = list(ckp.keys())
print(ckp_keys)
ckp = ckp['model']
simplified_ckp = {}
param_shapes = {key: ckp[key].shape for key in ckp.keys()}
...
for key in ckp.keys():
    if 'conv1_1' in key:
        if 'weight' in key:
            if ckp[key].shape[2] == 3 and ckp[key].shape[3] == 3:
                w_cdc = convert_cdc(ckp[key])
            else:
                print(f"Skipping {key} due to unsupported kernel size: {ckp[key].shape}")
                continue
        elif 'bias' in key:
            b_cdc = ckp[key]
    elif 'conv1_2' in key:
    ...
    torch.save(simplified_ckp, saved_model_path.split('/')[-1].split('.')[0] + '_simplified.pth.tar')